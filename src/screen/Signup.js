...
e.preventDefault()                                                                              //synthetic event  prevents all the default behavior by the browser.
 const response = await fetch("http://localhost:5000/api/createuser", {
  ...
  body:JSON.stringify({name:credentials.name,  password:credentials.password,email:credentials.email, location:credentials.geolocation})// to send data in backend
 });
